...
pygame.font.init()
ubuntu_bold_graph = pygame.font.Font("Ubuntu-B.ttf",20)
ubuntu_bold_graph_initial = pygame.font.Font("Ubuntu-B.ttf",18)
...
class text_input(graphic.graphic,graphic.square):
    def __init__(self):
        graphic.graphic.__init__(self)
        graphic.square.__init__(self)
        #saving background
...
"""
    ...
    def cLogic  (self):
        ...
        if mx > self.X and mx < self.X + self.W and my > self.Y and my < self.Y + self.H:
            ...
            if (self.pressed_aux2 == False and pressed2):
                self.pressed2 = False
                
            self.pressed_aux = False
        self.pressed_aux2 = pressed2
        ...
        border_background = border.add_border(background, self.border_color,self.border_size,self.border_size,self.border_size,self.border_size)
        ...
        #blitting tile
        if (self.loops % self.intensity > self.intensity / 8.0 * 3.0 and self.pressed2):
        ...
        self.loops += 1
    def cGraphic(self):
        tex = OGL.GenerateTexture(self.surface)
        OGL.BlitTexture(tex.texture,tex.width,tex.height,self.X,self.Y)
